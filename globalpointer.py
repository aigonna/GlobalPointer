...
class GlobalPointer(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        #上一步已经转换为bsxseq_lenxent_type_size*inner_dim*2
        #按照最后一个维度拆分成ent_type_size个bs x seq_len inner_dim*2的tensor
...
class GlobalPointerSdrop(nn.Module):
    def __init__(self, encoder, ent_type_size, inner_dim, RoPE=True):
        # encodr: RoBerta-Large as encoder
        # inner_dim: 64
        # ent_type_size: ent_cls_num
...
class EffiGlobalPointerSdrop(nn.Module):
    def __init__(self, encoder, ent_type_size, inner_dim, RoPE=True):
        # encodr: bert model
        # inner_dim: 64
        # ent_type_size: ent_cls_num
        super(EffiGlobalPointerSdrop, self).__init__()
        self.encoder = encoder
        self.ent_type_size = ent_type_size
        self.inner_dim = inner_dim
        self.hidden_size = encoder.config.hidden_size
